...
export function NotifMsg ( {profile, name, requestType}: Props ) {
	...
	const handleAccept = async () => {
		console.log(name);
		try {
			const response = await axios.post(`http://localhost:3000/accept-friend/${name}`, null, {withCredentials: true})
			.then((response) => {
	...
	const handleRefuse = async () => {
		console.log(name);
		try {
			const response = await axios.delete(`http://localhost:3000/delete-request/${name}`, {withCredentials: true})
			sethide(false);
		}	
		catch (error) {
			console.log(error);
